# portal-kasa one-click installer for the Meta Portal (Windows).
#
...
#
# Usage:
#   .\install.ps1             install portal-kasa on the connected Portal (downloads the latest release)
...
function Resolve-ReleaseAsset {
  ...
  try {
    $rel = Invoke-RestMethod -Uri "https://api.github.com/repos/$($cfg["RELEASE_REPO"])/releases/latest" `
      -Headers @{ "User-Agent" = "portal-kasa-installer"; "Accept" = "application/vnd.github+json" }
...
function Install-App {
  if ($LocalApk) {
    ...
    if ($asset -and $asset.Url -and $asset.Name) {
      ...
      if ($fresh) {
        Step "Latest portal-kasa release already downloaded ($($asset.Name)) - using it"
      } else {
        Step "Downloading the latest portal-kasa release ($($asset.Name))"
      ...
      if (-not $apk) { Die "No cached APK in apks\ and couldn't reach GitHub to download one. Connect to the internet, drop a portal-kasa APK in apks\, or use -Local." }
      Warn "Couldn't reach GitHub - installing the cached APK ($($apk.Name)); it may not be the latest."
    }
  }
  Step "Installing portal-kasa ($($apk.Name))"
...
if ($Status) {
  ...
  Write-Host "  portal-kasa: $(if ($inst) {'installed'} else {'not installed'})"
  exit 0
}

if ($Uninstall) {
  Write-Host "portal-kasa uninstaller`n"
  Wait-Device
  Step "Stopping and removing portal-kasa"
  A shell am force-stop $cfg["PKG"] | Out-Null
  A uninstall $cfg["PKG"] | Out-Null
  Write-Host "`n[ok] Done. portal-kasa removed." -ForegroundColor Green
  exit 0
}

Write-Host "portal-kasa installer" -ForegroundColor White
Write-Host "Installs the Kasa smart-plug controller on your Portal and opens it.`n" -ForegroundColor DarkGray
Wait-Device
Install-App
Open-App
Write-Host "`n[ok] Done. Open Kasa Plugs on the Portal - plugs on the same Wi-Fi are listed automatically." -ForegroundColor Green
Write-Host "For voice: enable 'Kasa Plugs' in the assistant's Settings > External tools, then start a new chat." -ForegroundColor DarkGray
Write-Host "To remove it: run install.ps1 -Uninstall (or double-click Uninstall-PortalKasa)." -ForegroundColor DarkGray
