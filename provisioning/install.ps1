...
function Install-App {
  ...
  if (-not $apk) {
    ...
    if (-not $url) { Die "No local APK in apks\ and couldn't find a release to download. Connect to the internet, or drop a portal-kasa APK in the apks\ folder." }
    Step "Downloading the latest portal-kasa release"
    $dir = Split-Path -Parent $cfg["APK_GLOB"]
    New-Item -ItemType Directory -Force -Path $dir | Out-Null
    $dest = Join-Path $dir "portal-kasa.apk"
