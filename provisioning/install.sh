...
# version-stamped asset name (e.g. portal-kasa-v1.0.0.apk), and its sha256 digest (the last may be
  ...
  if [ -n "$LOCAL_APK" ]; then
    ...
    if [ -n "$url" ] && [ -n "$name" ]; then
      ...
      if [ -f "$apk" ] && { [ -z "$digest" ] || [ -z "$have" ] || [ "$have" = "$digest" ]; }; then
        step "Latest portal-kasa release already downloaded ($name) — using it"
      else
        step "Downloading the latest portal-kasa release ($name)"
        mkdir -p "$cachedir"
      ...
      [ -n "$apk" ] || die "No cached APK in apks/ and couldn't reach GitHub to download one. Connect to the internet, drop a portal-kasa APK in apks/, or use --local."
